...
class iOSBuilder(object):
    ...
    def getBuildParams(self, project, target, workspace, scheme):
        ...
        if project is None and workspace is None:
            ...
            self._package_name = "{0}_{1}_v{2}_b{3}".format(target, self.configuration, self.build_version, self.build_date)
            self._app_name = target
        elif workspace is not None:
            build_params = '-workspace %s -scheme %s' % (workspace, scheme)
            # specify package name
            self._package_name = "{0}_{1}_v{2}_b{3}".format(scheme, self.configuration, self.build_version, self.build_date)
